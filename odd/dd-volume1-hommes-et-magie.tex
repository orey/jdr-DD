...
\phantomsection\label{sort-clairvoyance}\textbf{Clairvoyance} : comme le sort \uline{Perception extrasensorielle} excepté que le lanceur du sort peut simplement visualiser au lieu de détecter des pensées.

\bigskip

\phantomsection\label{sort-clairaudience}\textbf{Clairaudience} : comme le sort de \uline{Clairvoyance} excepté qu'il permet d'entendre plutôt que de voir. C'est un des rares sorts qui peut être lancé au travers d'une boule de cristal\footnote{Voir page \pageref{objet-boule-cristal} (NdT).}.
